...
IndirectWABSStream = function(options, callback) {
  var self = this;

  // Use new to fire up this baby
  if (!(this instanceof IndirectWABSStream))
    return new IndirectWABSStream(options);
  ...
  if (typeof callback !== 'function')
    throw new Error('WABS SA IndirectWABSStream needs callback');
  ...
  self.tempName = temp.path({ suffix: '.cfsWABS.bin'});
